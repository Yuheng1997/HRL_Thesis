...
def load_data(batch_size=128, device='cpu', path=None, shuffle=True):
...
def get_hitting_data(batch_size=128, device='cpu', path=None, shuffle=True, split=.9):
    if path is None:
        dataset_path = Config.data.path
    else:
        dataset_path = path
...
def get_resample_data(batch_size=128, device='cpu', path=None, shuffle=True, split=.9):
    if path is None:
        dataset_path = Config.data.path
    else:
        dataset_path = path
...
def update_dataloader(dataloader, new_data, batch_size=Config.train.batch_size, shuffle=True, device=Config.train.device):
