...
from high_level.utils.bspline import BSpline
...
class NNAgent(AgentBase):
    def __init__(self, env_info, model, **kwargs):
        ...
        self.ee_pos = None
        self.cur_high_action = np.zeros(4)
        # flag
        self.update_goal_and_gene_traj = True
    ...
    def validate_condition(self, obs):
        ...
        if len(self.traj_buffer):
            self.update_goal_and_gene_traj = False
        else:
            self.update_goal_and_gene_traj = True
    ...
    def update_goal(self, high_action):
        if self.cur_high_action == high_action:
            return
        else:
            self.hit_pos = high_action[:2]
            hit_angel = high_action[2]
            self.scale = high_action[-1]
            self.hit_vel = np.array([np.sin(hit_angel), np.cos(hit_angel)])
            self.cur_high_action = high_action
            self.update_goal_and_gene_traj = True
            self.traj_buffer = []
    ...
    def draw_action(self, _obs):
        high_action = _obs[-4:]
        obs = _obs[:23]
        self.validate_condition(obs)
        self.update_goal(high_action)
        save_and_fit = False
        if self.update_goal_and_gene_traj:
            self.traj_buffer = self.generate_hit_traj(obs)
        ...
        try:
            ...
            print('traj', len(self.traj_buffer), self.traj_buffer[0], 'high_action', self.cur_high_action, 'ee_pos', self.ee_pos)
    ...
    def generate_hit_traj(self, obs):
        ...
        return np.concatenate((pos_traj, vel_traj), axis=2).squeeze().tolist()
