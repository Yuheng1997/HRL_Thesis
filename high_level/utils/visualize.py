...
if __name__ == '__main__':
    ...
    checkpoint_path = os.path.join(os.path.abspath(os.getcwd()), '..\\model\\checkpoints\\Model_29800.pt')
    ...
    training_loader, validation_loader = load_data(batch_size=1, device=device,
                                                   path=os.path.join(
                                                       os.path.abspath(os.getcwd()),
                                                       "../datasets/train/data.tsv"), shuffle=False)
    ...
    for k, data in enumerate(validation_loader):
        ...
        dt = t_cumsum.cpu().detach().numpy()[0]
        q = q[:, :].cpu().detach().numpy()[0]
        q_dot = q_dot[:, :].cpu().detach().numpy()[0]
        q_ddot = q_ddot[:, :].cpu().detach().numpy()[0]
        ...
        ts = np.linspace(0, max(dt), int(max(dt) * 1000))
        ...
        action = np.stack([pos, vel, acc], axis=1)

        urdf_file = os.path.join(os.getcwd(), "../iiwa_only.urdf")
        manipulator = DifferentiableRobotModel(urdf_path=urdf_file,
                                               tensor_args={'device': device, 'dtype': torch.float32})
        positions, rotations = manipulator.compute_forward_kinematics(
            torch.from_numpy(action[:, 0, :]).to(torch.float32),
            torch.from_numpy(action[:, 1, :]).to(torch.float32),
            link_name='iiwa_1/striker_tip')

        positions = positions.cpu().detach().numpy()
        rotations = rotations.cpu().detach().numpy()

        env.init_state = np.array(action[0, 0])
        env.reset()

        i = 0
        while i < int(max(dt) * 1000):
            if i + 20 <= len(action):
                a = action[i:i+20]
            else:
                a = np.zeros((20, 3, 7))
                a[:, 0, :] = action[-1, 0, :]
                a[20-(len(action) % 20):] = action[i:]

            # a = [action[0] for i in range(20)]
