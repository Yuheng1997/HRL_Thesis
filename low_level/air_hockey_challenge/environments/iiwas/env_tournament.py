...
class AirHockeyTournament(AirHockeyDouble):
    ...
    def viewer_tactic_update(self, tactic, skill):
        self.tactic = f"{tactic}, {skill}"

    def setup(self, obs):
        # Initial position of the puck
        puck_pos = np.random.rand(2) * (self.hit_range[:, 1] - self.hit_range[:, 0]) + self.hit_range[:, 0]
        min_t = 0.8
        dis_x = puck_pos[0] - (-0.7)
        dis_yl = 0.45 - puck_pos[1]
        dis_yr = (-0.45) - puck_pos[1]
        max_vel_x = dis_x / min_t
        max_vel_yl = dis_yl / min_t
        max_vel_yr = dis_yr / min_t
        lin_vel_x = -np.random.uniform(0, max_vel_x)
        lin_vel_y = -np.random.uniform(-max_vel_yl, -max_vel_yr)
        puck_vel = np.zeros(3)
        # puck_vel[0] = lin_vel_x
        # puck_vel[1] = lin_vel_y
        # puck_vel[2] = np.random.uniform(-5, 5, 1)
